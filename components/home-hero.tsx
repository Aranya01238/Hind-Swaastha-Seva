...
Status: HealthSecure Live - Encrypted Session Active
...
├─ Date:             ${appt.date}
├─ Time:             ${appt.time}
└─ Status:           ${appt.status.toUpperCase()} ${appt.status === 'confirmed' ? '✓' : '⏳'}
`).join('')}

═══════════════════════════════════════════════════════════════════════════════
                            CLINICAL TEST INVENTORY
═══════════════════════════════════════════════════════════════════════════════
...
├─ Date:             ${test.date}
├─ Result:           ${test.result}
└─ Status:           ${test.status.toUpperCase()} ${test.status === 'stable' ? '✓' : '⚠️'}
`).join('')}
...
Available Services:
• Consult Doctor       - Schedule new appointments
• Find Beds            - Check hospital bed availability  
• Blood Bank           - Access blood donation services
• Laboratories         - Book diagnostic tests
...
© 2025 HealthSave Security Cluster
This document contains confidential medical information.
Handle according to HIPAA and local privacy regulations.

Document ID: HSS-${Date.now()}
Generated by: HealthSave Platform v2.2.256
      `.trim();
      ...
      const safeName = user.name ? user.name.replace(/\s+/g, '_') : 'User';
      const dateStr = new Date().toISOString().split('T')[0];
      link.download = `HealthID_${safeName}_${dateStr}.txt`;
      ...
      console.error('Error generating health ID file:', error);
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-red-50 selection:text-red-600">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 relative z-10 mt-20">
        {/* DASHBOARD HEADER */}
        <header className="mb-12 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 animate-fade-in-up">
          <div className="space-y-1">
            ...
            <h1 className="text-4xl lg:text-5xl font-black tracking-tight text-slate-900 leading-tight">
              Welcome,{" "}
              <span className="text-red-600 italic font-serif font-light underline decoration-red-200 underline-offset-8 decoration-4">
                {MOCK_NAME.split(" ")[0]}
              </span>
            </h1>
            <p className="text-slate-500 font-medium max-w-xl">
              Track your recovery, manage appointments, and access your clinical
              reports with end-to-end encryption.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto">
            <button 
              ...
              className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-6 py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-red-600 transition-all shadow-xl shadow-slate-900/10 active:scale-95"
            >
            ...
            <section className="bg-white border border-slate-100 p-8 rounded-[2.5rem] shadow-2xl shadow-slate-200/40 relative overflow-hidden group">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div
                    key={i}
                    className="flex items-center gap-4 p-4 bg-slate-50/50 rounded-2xl border border-slate-100 hover:border-red-200 hover:bg-white transition-all"
                  >
                    <item.icon
                      size={18}
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <button className="flex flex-col items-center gap-3 p-7 bg-white border border-slate-100 rounded-[2rem] hover:border-red-500 transition-all shadow-sm group">
                  <BedDouble
                    className="text-slate-200 group-hover:text-red-600 transition-colors"
                    size={28}
                  />
          ...
          <div className="lg:col-span-8 space-y-8 animate-fade-in-up delay-200">
            ...
            <section className="bg-white border border-slate-100 rounded-[2.5rem] p-8 lg:p-10 shadow-sm relative overflow-hidden">
              <div className="flex items-center justify-between mb-10">
                <h3 className="text-sm font-black uppercase tracking-[0.2em] flex items-center gap-3 text-slate-400">
                  <Calendar size={20} className="text-red-600" /> Scheduled
                  Consultations
                </h3>
                <div className="h-[1px] flex-grow mx-8 bg-slate-100 hidden sm:block" />
                <button className="text-[10px] font-black text-red-600 uppercase tracking-widest hover:underline">
                  Full History
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {appointments.map((appt) => (
                  <div
                    key={appt.id}
                    className="relative p-8 bg-slate-50 border border-slate-100 rounded-[2rem] group hover:border-red-200 hover:bg-white transition-all shadow-sm hover:shadow-2xl"
                  >
                    <div className="flex justify-between items-start mb-6">
                      ...
                      <span
                        className={`text-[10px] font-black px-3 py-1.5 rounded-xl uppercase tracking-widest border shadow-sm ${
                    ...
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.1em] mb-8">
                      {appt.specialty}
                    </p>
                    <div className="flex items-center justify-between pt-6 border-t border-slate-200/60">
                      ...
                      <div className="text-xs font-black text-slate-950 uppercase tracking-widest">
                        {appt.date}
                      </div>
                    </div>
                  </div>
            ...
            <section className="bg-white border border-slate-100 rounded-[2.5rem] p-8 lg:p-10 shadow-sm relative overflow-hidden">
              <div className="flex items-center gap-4 mb-10">
                <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400 whitespace-nowrap">
                  Clinical Test Inventory
                </h3>
              ...
              <div className="space-y-4">
                {labTests.map((test) => (
                  <div
                    key={test.id}
                    className="flex flex-col md:flex-row items-center justify-between p-8 border border-slate-100 rounded-[2rem] hover:bg-slate-50 hover:border-red-200 transition-all group"
                  >
                    <div className="flex items-center gap-6 flex-1 w-full mb-6 md:mb-0">
                      ...
                      <div>
                        ...
                        <h4 className="font-black text-slate-900 text-xl group-hover:text-red-600 transition-colors tracking-tight">
                          {test.name}
                        </h4>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-10 w-full md:w-auto border-t md:border-t-0 pt-6 md:pt-0 border-slate-100">
                      <div className="text-right flex-grow md:flex-grow-0">
                        <p className="text-[9px] font-black text-slate-300 uppercase mb-1 tracking-widest">
                          Observation
                        </p>
                        <div
                          className={`flex items-center gap-2 justify-end font-black text-sm uppercase tracking-widest ${
                            test.status === "stable"
                              ? "text-green-600"
                              : "text-red-600"
                          }`}
